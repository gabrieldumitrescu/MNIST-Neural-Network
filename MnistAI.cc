...
{ 	
	...
	std::vector<size_t> layerSizes = {784,30,10};
	NetworkParameters params(CrossEntropyCostFunction,L2Regularization,true);
	Network nNet(layerSizes,params);
	...
	size_t batchSz=10;
	float lambda=5.0f;
	float reg_factor=(1.0 - ((eta * lambda) / (float) trainInputs.size()));
  size_t correct=0;
	for (int i = 0; i < numEpochs; ++i)
	{
		printf("Training epoch %i.....\n",i+1);
		nNet.trainNetwork(trainInputs,trainLabels,eta,batchSz,reg_factor);
		size_t currentCorrect = nNet.evaluate(validInputs, validLabels);
...
bool initWindow(SDL_Window* &wnd)
{
	...
	wnd=SDL_CreateWindow("Neural Network View", SDL_WINDOWPOS_UNDEFINED,SDL_WINDOWPOS_UNDEFINED,SCREEN_WIDTH,SCREEN_HEIGHT,SDL_WINDOW_SHOWN);
	if(wnd==NULL)
	{
